...
void Timer_32_Handler(void * arg){
	...
	hal_printf("Node %d's current random value is %u\n", g_OMAC.GetAddress(), g_DataTransmissionHandler.GetCurrentRandomValue());
	hal_printf("Neighbor node %d's current random value is %u\n", Neighbor2beFollowed, g_DataReceptionHandler.GetCurrentRandomValue());
	...
	UINT64 currentTicks = HAL_Time_CurrentTicks();
	hal_printf("-----------------\n");
	hal_printf( "[Local: %llu Neighbor: %llu]\n", currentTicks, g_omac_scheduler.m_TimeSyncHandler.m_globalTime.Local2NeighborTime(Neighbor2beFollowed, currentTicks) );
	hal_printf("m_nextWakeupSlot: %u\n", m_nextWakeupSlot);
	hal_printf("m_nextTXTicks: %u\n", m_nextTXTicks);
	hal_printf("m_nextTXCounter: %u\n", m_nextTXCounter);
	hal_printf("current slotNumber: %u\n",g_omac_scheduler.GetSlotNumber());
	hal_printf("-----------------\n");
	INT64 tmp_nbrGlobalTime = g_DataTransmissionHandler.GetNbrGlobalTime();
	if(tmp_nbrGlobalTime > 0){
		hal_printf("Neighbor global time: %lld\n",tmp_nbrGlobalTime);
	}
	hal_printf("Neighbor wakeup slot: %u\n",g_DataTransmissionHandler.GetNbrWakeupSlot());
	hal_printf("-----------------\n");*/
...
BOOL OMACTest::Initialize(){
	...
	VirtTimer_SetTimer(32, 0, 2*ONEMSEC_IN_USEC*ONESEC_IN_MSEC, FALSE, FALSE, Timer_32_Handler); //period (3rd argument) is in micro seconds
	return TRUE;
}
