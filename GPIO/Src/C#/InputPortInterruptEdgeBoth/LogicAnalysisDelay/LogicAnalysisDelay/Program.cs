...
namespace LogicAnalysis
{
    class Program
    {
        ...
        static int FindTransitionToLow(List<int> time, List<int> line, int initialTime)
        {
            int returnVal = 0;
            int index = 1;
            ...
            while ((index < time.Count))
            {
                if ((line[index - 1] == 1) && (line[index] == 0))
                    break;
                index++;
            }
            ...
            try
            {
                ...
                if (listNumber != 3)
                {
                    ...
                    using (StreamWriter writer = new StreamWriter(fileOut))
                    {
                        ...
                        writer.Write("resultParameter1 = Bad data format in reading from file.\r\n");
                ...
                int transitionTime = FindTransitionToLow(listTime, line1, 0);
                if (transitionTime == 0)
                {
                    System.Diagnostics.Debug.WriteLine("Error in Main");
                    using (StreamWriter writer = new StreamWriter(fileOut))
                ...
                // Get 0 -> 1 transition on line1 from the beginning 
                int transitionTimeHigh = FindTransitionToHigh(listTime, line1, gapTime);
                if (transitionTimeHigh == 0)
                {
                    System.Diagnostics.Debug.WriteLine("Error in Main");
                    using (StreamWriter writer = new StreamWriter(fileOut))
                    {
                ...
                // Find 0 -> 1 transition on line1 from the transition time found above
                int gapTimeHigh = FindTransitionToHigh(listTime, line0, transitionTimeHigh);

                float timeToInterruptLow = ((float)gapTime - (float)transitionTime) / 4000000;
                float timeToInterruptHigh = ((float)gapTimeHigh - (float)transitionTimeHigh) / 4000000;

                if ((timeToInterruptLow < 0.01) && (timeToInterruptLow > 0) && (timeToInterruptHigh < 0.01) && (timeToInterruptHigh > 0))
                {
                    testResult = true;
                }
                else
                ...
                using (StreamWriter writer = new StreamWriter(fileOut,false))
                {
                    if (testResult == true)
                        ...
                        writer.Write("result = FAIL\r\n");
                    writer.Write("accuracy = " + (timeToInterruptLow / 0.0045) + "\r\n");
                    writer.Write("resultParameter2 = " + timeToInterruptHigh.ToString() + "\r\n");
                    writer.Write("resultParameter3 = " + timeToInterruptLow.ToString() + "\r\n");
                    writer.Write("resultParameter4 = null\r\n");
                    writer.Write("resultParameter5 = null\r\n");
                    writer.Write("resultParameter5 = null\r\n");
                    writer.Close();
