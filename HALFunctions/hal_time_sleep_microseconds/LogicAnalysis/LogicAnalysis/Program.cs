...
namespace LogicAnalysis
{
    class Program
    {
        ...
        const float accuracyFirstGap = 0.3f;
        const float accuracySecondGap = 0.2f;

        static List<double> expectedGapSequence0 = new List<double>() { 0.000005, 0.000022, 0.000265, 0.002420 };
        ...
        static float FindGap(List<int> time, List<int> line, List<double> expectedGapSequence, int startPoint, int stopPoint, int transitionType)
        {
            ...
            for (int i = samplePoint + 1; i < stopPoint; i++)
            {
                ...
                if (foundTransition == true)
                {
                    ...
                    {
                        if (currentGapSearchNum == 0)
                        {
                            if ((gapTime <= (expectedGapSequence[currentGapSearchNum] * (1 + accuracyFirstGap))) && (gapTime >= (expectedGapSequence[currentGapSearchNum] * (1 - accuracyFirstGap))))
                            {
                                currentGapSearchNum++;
                            }
                            else
                        ...
                        else if (currentGapSearchNum == 1)
                        {
                            if ((gapTime <= (expectedGapSequence[currentGapSearchNum] * (1 + accuracySecondGap))) && (gapTime >= (expectedGapSequence[currentGapSearchNum] * (1 - accuracySecondGap))))
                            {
                                currentGapSearchNum++;
                            }
                            else
