...
void* myReceiveHandler (void *msg, UINT16 Size){
	...
	UINT64 EventTime = PacketTimeSync_15_4::EventTime((Message_15_4_t *) msg, Size);

	hal_printf("Received Packet %d\n", EventTime);
...
BOOL RadioSlaveTest::TimeStamp_Reciever()
{
	DeviceStatus result;
...
BOOL RadioSlaveTest::Execute( int testLevel )
{
	...
	if(testLevel == TIMESTAMP)
		result = TimeStamp_Reciever();
