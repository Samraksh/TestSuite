...
UINT32 packetRecvCount = 0;
UINT32 lastMesgRecd = 0;

extern "C"
{

void* myReceiveHandler_CC(void *msg, UINT16 Size){
	...
	hal_printf("Packet found on channel %d\n", RadioTest::channel + 11);
	/*
	RadioTest::msgRecvAck.MSGID= 5;
	RadioTest::msgRecvAck.data[0] = 5;
	RadioTest::msgRecvAck.data[1] = 5;
	RadioTest::msgRecvAck.data[2] = 5;
	RadioTest::msgRecvAck.data[3] = 5;
	RadioTest::msgRecvAck.data[4] = 5;
	*/
	RadioTest::respond = TRUE;
	// Try sending a packet as an ack
	//SendResult = SendPacketSync(MAC_BROADCAST_ADDRESS, MFM_DATA, (void *) &RadioTest::msgRecvAck.data, sizeof(Payload_t));
...
void *myReceiverHandler_CP(void *msg, UINT16 Size)
{
	BOOL SendResult = FALSE;

	UINT8 *data = (UINT8 *) msg;
	lastMesgRecd = data[14];
	lastMesgRecd |= data[15] << 8;
	lastMesgRecd |= data[16] << 16;
	lastMesgRecd |= data[17] << 24;
...
void Timer_CP_Handler(void * arg)
{
		hal_printf("Number Of Packets received : %d\n", packetRecvCount);
		hal_printf("Number Of Packets Sent     : %d\n", lastMesgRecd);

}

void Timer_CC_Handler(void *arg)
{
	// Reset channel being scanned
	if(RadioTest::channel == 15)
		RadioTest::channel = -1;

	CPU_Radio_ChangeChannel(1, ++RadioTest::channel);

	hal_printf("Scanning Channel %d\n", RadioTest::channel + 11);
}
...
RadioTest::RadioTest( int seedValue, int numberOfEvents )
{
	...
	radioEventHandler.SetRecieveHandler(&myReceiverHandler_CP);
...
BOOL RadioTest::ChangeTxPowerTest_Reciever()
{
	UINT8 result;
	...
	gHalTimerManagerObject.CreateTimer(1, 0, 1000000, FALSE, FALSE, Timer_CP_Handler); //1 sec Timer in micro seconds
...
BOOL RadioTest::ChangeChannelTest_Reciever()
{
	UINT8 result;
	...
	gHalTimerManagerObject.CreateTimer(1, 0, 1000000, FALSE, FALSE, Timer_CC_Handler); //1 sec Timer in micro seconds

	if(CPU_Radio_ChangeChannel(radioID, channel) != DS_Success)
	{
		hal_printf("Changing the channel initially failed");
		return FALSE;
	}
	...
	while(TRUE)
	{
		HAL_Time_Sleep_MicroSeconds(750);

		if(RadioTest::respond == TRUE)
		{
			RadioTest::respond = FALSE;
			...
			hal_printf("Responding to master on channel %d\n", channel + 11);
...
BOOL RadioTest::SleepTest_Reciever()
{
	...
	gHalTimerManagerObject.CreateTimer(1, 0, 10000000, FALSE, FALSE, Timer_1_Handler); //1 sec Timer in micro seconds
...
BOOL RadioTest::Execute( int testLevel )
{
	...
	else if(testLevel == CHANGECHANNEL)
	{
		result = ChangeChannelTest_Reciever();
	}
	else if(testLevel == CHANGEPOWER)
	{
		result = ChangeTxPowerTest_Reciever();
	}
