    ...
    public class DataStoreTest
    {
        ...
        public DataStoreTest()
        {
            try
            {
                ...
                experimentIndex = 100;
                bufferSize = 1016;  //(1016*2(UINT16))+16(header size)=2048
                rand = new Random();
                //data = new Data[experimentIndex];
                //offsetIndex = 128;
                //offsetIndex = (UInt16)(rand.Next((int)bufferSize / 2));

                readBuffer = new UInt16[bufferSize];
                writeBuffer = new UInt16[bufferSize];
                for (UInt16 writeIndex = 0; writeIndex < writeBuffer.Length; ++writeIndex)
                {
                    writeBuffer[writeIndex] = (UInt16)(writeIndex + 1);
                }
            }
            catch (Exception ex)
            {
                Debug.Print(ex.Message);
                DisplayStats(false, "Test Level_6B failed", "", 0);
                return;
            }
        }
        ...
        public bool StartReading()
        {
            ...
            {
                DisplayStats(false, "ReadAllDataReferences - test Level_6B failed", "", 0);
                return false;
            }
            ...
            while (offset < ObjectCount)
            {
                ...
                if (dataRefArray[dataIndex-1].Read(readBuffer, 0, readBuffer.Length) != DataStoreReturnStatus.Success)
                {
                    if ((offset + dataIndex-1) > ObjectCount) {
                        ...
                        DisplayStats(false, "Read not successful - test Level_6B failed", "", 0);
                        return false;
                    }
                }
                ...
                for (UInt16 rwIndex = 0; rwIndex < readBuffer.Length; ++rwIndex)
                {
                    ...
                    {
                        DisplayStats(false, "Read Write test failed - test Level_6B failed", "", 0);
                        return false;
                    }
                }
        ...
        public void Level_6B()
        {
            try
            {
                ...
                {
                    data = new DataReference(dStore, bufferSize, ReferenceDataType.UINT16);
                    ObjectCount++;
                    ...
                    if (data.Write(writeBuffer, 0, writeBuffer.Length) != DataStoreReturnStatus.Success)
                    {
                        ...
                        {
                            DisplayStats(false, "Data write failure - test Level_6B failed", "", 0);
                            return;
                        }
                        else
                        {
            ...
            catch (DataStoreOutOfMemoryException ex)
            {
                ...
                if (readResult)
                {
                    ...
                    DisplayStats(true, "Test Level_6B successfully completed", "", 0);
                }
                else
                {
                    DisplayStats(false, "Test Level_6B failed", "", 0);
        ...
        public static void Main()
        {
            ...
            Debug.Print(DateTime.Now.ToString());
            Debug.Print("Starting test Level_6B");
            dtest.Level_6B();
        }
    }
}
