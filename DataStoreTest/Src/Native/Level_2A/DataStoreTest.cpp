...
LPVOID GetAddress(RECORD_ID rId)
{
	LPVOID retVal = NULL;
...
UINT16 GenerateRandomNumber(UINT16 upperBound)
{
...
BOOL DataStoreTest::TestReadWrite_Random_Record()
{
	...
	{
		write_data[rwIndex] = (UINT16)(testMathInstance.pareto_prng() % (1 << 7));
	}
...
BOOL DataStoreTest::Execute( int testLevel )
{
	...
	switch(testLevel)
	{
		case 0:
			...
			break;
		case 1:
			result = TestReadWrite_Random_Record();
			break;
		case 2:
