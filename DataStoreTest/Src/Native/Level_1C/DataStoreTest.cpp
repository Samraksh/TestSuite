...
//const double definedFlashSize = (1.0 * 131072.0);     //Size of the flash is: 125 * 131072 = 16384000 (~16MB). Little more than actual flash size in order to check if exception is thrown.
//const double definedFlashSize = g_dataStoreObject.returnTotalSpace() + 5000;

const int objectSize = 100;
UINT32 test_limit = objectSize;
UINT32 objectCount = 16384000;

BOOL endOfObjectsFlag = false;
UINT32 finalObjectID = 0;
...
LPVOID DataStoreTest::CreateDataStoreRecords(int count, int i)
{
	...
	for(UINT32 index = 1+(i-1)*count; index <= i*count; index++)
	{
...
BOOL DataStoreTest::StartReading()
{
	...
	for(UINT32 index = 1; index <= objectCount; index+=100)
	{
		if(index % 501 == 0) {
			hal_printf("index=%d\n", index);
		}
...
BOOL DataStoreTest::Test_FillFlash()
{
	...
	while(flashUsedSpace <= definedFlashSize)
	{
		...
		for(UINT32 index = 1+((i-1)*objectsToBeCreated); index <= i*objectsToBeCreated; index++)
		//for(UINT32 index = 15500; index <= 902912; index++)
		{
			//if(index >= 901800) {
				//hal_printf("%d\n", index);
			//}
