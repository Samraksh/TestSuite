...
BOOL VirtualTimerTest::Level_0E()
{
	...
	for(int i = 0; i < 1; i++)
	{
		...
		if(!VirtTimer_SetTimer(2, 0, periodValue*1000, FALSE, FALSE, Timer_2_Handler))
			return FALSE;*/
		//periodValue = (i+2)%9;
		if(!VirtTimer_SetTimer(4, 0, periodValue*11000, FALSE, FALSE, Timer_0_Handler))
			return FALSE;
		//periodValue = (i+1)%9;
		if(!VirtTimer_SetTimer(5, 0, periodValue*23000, FALSE, FALSE, Timer_1_Handler))
			return FALSE;
		//periodValue = (i+2)%9;
		if(!VirtTimer_SetTimer(6, 0, periodValue*37000, FALSE, FALSE, Timer_2_Handler))
			return FALSE;
		//periodValue = (i+1)%9;
		if(!VirtTimer_SetTimer(7, 0, periodValue*41000, FALSE, FALSE, Timer_3_Handler))
			return FALSE;
	}
