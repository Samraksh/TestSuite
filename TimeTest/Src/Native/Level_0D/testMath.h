...
public:
	...
	void prng_init(int SeedValue)
	{
		...
		multiplier = 4096; // 2 ^ 12
		increment = 12163;
		modulus = 16216;
		seedValue = SeedValue;
		//randomValue = (double) SeedValue;
		randomValue = SeedValue;
	}
	...
	unsigned int prng()
	{
		...
		//intermedResult = (4096 * randomValue + increment) % (modulus);
		intermedResult = (4096 * randomValue + increment) / (modulus);
...
//n		randomValuecdf = pow((float)(100 / randomValue),2);
	...
	UINT16 pareto_prng()
	{
		...
		unsigned int squareRoot = (sqrroot_fp((1 << 12) - randomValue) / (1 << 8));
		//sqrrootValueFP = sqrroot((1 << 24) - randomValue);
		//resultFP = (((1 << 24) / (int) squareRoot) * 500);
		resultFP = (((1 << 12) / (int) squareRoot) * 500);
