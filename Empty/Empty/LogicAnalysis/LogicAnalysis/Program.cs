...
namespace LogicAnalysis
{
    class Program
    {
        ...
        static List<double> listTime = new List<double>();
        ...
        static void ReadInFile()
        {
            ...
            while (!reader.EndOfStream)
            {
                ...
                //System.Diagnostics.Debug.WriteLine(int.Parse)
                listTime.Add(double.Parse(values[0]));
        ...
        static bool GetJitter(List<double> time, List<int> line, int startPoint, int stopPoint, ref double minTime, ref double maxTime, ref double stdDeviation)
        {
            bool returnValue = false;

            // skipping first few transitions as they skew results
            int samplePoint = startPoint;
            int skipTransitions = 1000;
            ...
            lastPoint = line[samplePoint];
            double lastTransitionTime = time[samplePoint];            
            for (int i = samplePoint + 1; i < stopPoint; i++)
            {
                ...
                if ((line[i] == 1) && (lastPoint == 0))
                {
                    ...
                    if ((transitionTime < (0.039*sampleFreq)) && (transitionTime > (0.03*sampleFreq)))
                    {
                        System.Diagnostics.Debug.WriteLine("***************" + (transitionTime/sampleFreq).ToString() + " " + i.ToString());
                    }
            ...
            if ((totalTransistions != 0) && (totalTime != 0))
                calcAverage = (float)((totalTime / totalTransistions));
            ...
            for (int i = samplePoint + 1; i < stopPoint; i++)
            {
                ...
                if ((line[i] == 1) && (lastPoint == 0))
                {
                    transitionTime = time[i] - lastTransitionTime;
                    lastTransitionTime = time[i];
                    varianceNumerator += (float)((transitionTime - calcAverage) * (transitionTime - calcAverage));
                    totalTransistions++;
                }
                lastPoint = line[i];
            }
            ...
            System.Diagnostics.Debug.WriteLine("Max: " + (maxTime * 1000).ToString() + " Min: " + (minTime * 1000).ToString() + " diff: " + ((maxTime - minTime) * 1000).ToString() + " stdDev: " + stdDeviation.ToString());
            System.Diagnostics.Debug.WriteLine((maxTime * 1000).ToString() + " " + (minTime * 1000).ToString() + " " + ((maxTime - minTime) * 1000).ToString() + " " + stdDeviation.ToString());
                       
            return returnValue;
        }

        static float GetFrequency(List<double> time, List<int> line, int startPoint, int stopPoint)
        {
            float calcFreq = 0;
            ...
            double totalTime = 0;
            double totalTransistions = 0;
            double lastPoint = line[samplePoint];
            double lastTransitionTime = time[samplePoint];
            for (int i = samplePoint + 1; i < stopPoint; i++)
            {
                // looking for transition from 0 to 1
            ...
            if ((totalTransistions != 0) && (totalTime != 0))
                calcFreq = (float)(sampleFreq / (totalTime / totalTransistions));
        ...
        static void Main(string[] args)
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine("Error in Main: " + ex.ToString());
