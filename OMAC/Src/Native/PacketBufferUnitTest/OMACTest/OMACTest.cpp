...
void FAAVLBSTTest::PrintElemnentsofTreeStructured(FAAVLBSTElement<Message_15_4_t> *curElement, FAAVLBSTHeight_t offsetheight=0 ){
	uint16_t x = 0xFFFFF;
	...
	if(curElement){
		...
		if (curElement->parent) {
			hal_printf("%d(%d_%d_%d)",curElement->data.GetHeader()->dest, curElement->parent->data.GetHeader()->dest,curElement->DepthL(),curElement->DepthR());
		}
		else{
			hal_printf("%d(--_%d_%d)",curElement->data.GetHeader()->dest,curElement->DepthL(),curElement->DepthR());
		}
		hal_printf("\n");
...
void FAAVLBSTTest::PrintElemnentsofTreeDepthFirst(FAAVLBSTElement<Message_15_4_t> *curElement ){
	hal_printf("CurElement %d \n", curElement->data.GetHeader()->dest);
	if(curElement->left) {
	...
	if(curElement->parent) {
		hal_printf("%d 's parent %d \n", curElement->data.GetHeader()->dest, curElement->parent->data.GetHeader()->dest);
	}else{
		hal_printf("%d has no Parent\n", curElement->data.GetHeader()->dest);
	}
	hal_printf("%d DepthL() = %d DepthR() = %d \n",curElement->data.GetHeader()->dest, curElement->DepthL(), curElement->DepthR());
	fflush(stdout);
}
...
uint32_t cur_num_items = 0;
 ...
 hal_printf("Inserting uint16_t ... \n");fflush(stdout);
  for(uint16_t i = l1; i >= 1; i--)
  {
    for(uint16_t j = (i * l2) - l2+1 ; j <= i * l2; j++)
    {
     hal_printf("Inserting %d NumElemnents = %d \n", j, shortFAAVLBST->Size());fflush(stdout);
      if(! shortFAAVLBST->InsertPacketWithDest(j) ) {
  ...
  if(shortFAAVLBST->InsertPacketWithDest(NUMITEMS+1) ) {
  ...
  for(FAAVLBSTElement<Message_15_4_t>* i = shortFAAVLBST->Begin(); i != NULL; i = shortFAAVLBST->Next(i) ){
		hal_printf( "CurElement = %d ", i->data.GetHeader()->dest);fflush(stdout);
  }
  hal_printf( "\n ");

  hal_printf( "Traversing from end to begin\n ");
  for(FAAVLBSTElement<Message_15_4_t>* i = shortFAAVLBST->End(); i != NULL; i = shortFAAVLBST->Previous(i) ){
		hal_printf( "CurElement = %d ", i->data.GetHeader()->dest);fflush(stdout);
  }
  hal_printf( "\n ");
  ...
  for(uint16_t i = l1; i >= 1; i--)
  {
    for(uint16_t j = (i * l2) - l2+1 ; j <= i * l2; j++)
    {
    	if(j %2 ){
    		  ...
    		  if(shortFAAVLBST->SearchPacketWithDest(j)){
    			 hal_printf( "\n Extracting %d NumElemnents = %d ... \n", j, shortFAAVLBST->Size());fflush(stdout);
    			  if(!shortFAAVLBST->DeletePacketWithDest(j)) {
  ...
  while(shortFAAVLBST->Size() > 0){
	  ...
	  if(shortFAAVLBST->SearchPacketWithDest(value)){
		 hal_printf( "\n Extracting %d NumElemnents = %d ... \n", value, shortFAAVLBST->Size());fflush(stdout);
		  if(!shortFAAVLBST->DeletePacketWithDest(value)) {
			 hal_printf( "\n TEST FAIL: Delete failed for a found element =  %d NumElemnents = %d ... \n", value, shortFAAVLBST->Size());fflush(stdout);
		      hal_printf( "\n runFAAVLBST ********BEGIN Tree*********\n");
		      //rintElemnentsofTreeDepthFirst(shortFAAVLBST->GetRootPtr());
		      PrintElemnentsofTreeStructured(shortFAAVLBST->GetRootPtr(),0);
...
if(shortFAAVLBST->Size() != 0){
 ...
 if(! shortFAAVLBST->InsertPacketWithDest(test_element) ) {
	 hal_printf("TEST FAIL: Insertion FAILED \n");fflush(stdout);
 }
 else if(shortFAAVLBST->Size() != ++cur_num_items){
	 hal_printf("TEST FAIL : NUM ITEMS NOT INCREMENTED  FAILED \n");fflush(stdout);
...
//    if(shortFAAVLBST->InsertPacketWithDest(value)){
//    	++l1;
//    }
//    hal_printf( "\n runFAAVLBST ********BEGIN Tree*********\n");
//    PrintElemnentsofTreeStructured(shortFAAVLBST->GetRootPtr(),0);
